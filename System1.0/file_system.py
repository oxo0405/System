...
class FCB:
    TYPE_FILE = 0
    TYPE_DIRECTORY = 1
    def __init__(self, name, ftype, uid=0, gid=0):
        self.name = name
        self.type = ftype
        self.size = 0
...
class FileSystem:
    def __init__(self, data_file="filesystem_data.json"):
        ...
        self.MAX_CACHE = 50
        self.cache_mem = 0
        self.MAX_CACHE_MEM = 5*1024*1024
    ...
    def _init_fs(self):
        ...
        # 直接操作：创建 /home 目录
        home = FCB("home", FCB.TYPE_DIRECTORY, 0, 0)
        home.permission = 0o755
        home.content = json.dumps([])
        entries = json.loads(root.content)
        entries.append(home.to_dict())
        root.content = json.dumps(entries)

        # 注册默认用户（会自动创建 /home/admin, /home/guest）
    ...
    def _find_fcb(self, path):
        ...
        for part in parts:
            found = None
            entries = self._get_dir_entries(current)
            for e in entries:
                if e['name'] == part:
                    found = FCB.from_dict(e)
                    break
            if not found:
                return None
            current = found
    ...
    def _get_dir_entries(self, dir_fcb):
        if not dir_fcb or dir_fcb.type != FCB.TYPE_DIRECTORY:
            return []
        try:
            return json.loads(dir_fcb.content) if dir_fcb.content else []
        except:
            return []
    ...
    def register_user(self, username, password):
        if username in self.name_to_uid or len(username)<3 or len(password)<6:
            return False
        uid = self.next_uid; gid = self.next_gid
    ...
    def login_user(self, username, password):
        if username not in self.name_to_uid:
            return False
        uid = self.name_to_uid[username]
        user = self.users[uid]
    ...
    # ---------- 目录操作 ----------
    def mkdir(self, path):
        # 允许在初始化时创建目录（即使没有登录用户）
        # 但如果是用户登录后调用，必须已登录
        if self.current_user is None:
            # 如果是初始化阶段（没有用户），允许创建
            # 判断：如果 root 已存在且当前无用户，认为是初始化
    ...
    def rmdir(self, path):
        ...
        if not target or target.type != FCB.TYPE_DIRECTORY:
            return False
        if self._get_dir_entries(target):
            return False
    ...
    def ls(self):
        return self.list_directory(self.cwd_path)

    def list_directory(self, path):
        abs_path = self._resolve(path)
        fcb = self._find_fcb(abs_path)
        if not fcb or fcb.type != FCB.TYPE_DIRECTORY:
            return []
        entries = self._get_dir_entries(fcb)
        result = []
        for e in entries:
            f = FCB.from_dict(e)
    ...
    def open_file(self, path):
        ...
        if not target or target.type != FCB.TYPE_FILE:
            return -1
        # 简化权限检查：如果是当前用户创建的文件或所有者是自己，则允许读取
        # 否则检查权限
        if target.owner_uid != self.current_user.uid:
            if not target.check_permission(self.current_user.uid, self.current_user.gid, 0x01):
                return -1
        fd = self.fd_counter
        self.fd_counter += 1
        ...
        target.open_count += 1
        if target.content:
            self._cache_put(path, target.content)
        return fd
    ...
    def read_file(self, fd):
        ...
        if fcb.owner_uid != self.current_user.uid:
            if not fcb.check_permission(self.current_user.uid, self.current_user.gid, 0x01):
                return ""
    ...
    def write_file(self, fd, content, append=False):
        ...
        path, fcb = self.open_files[fd]
        if fcb.owner_uid != self.current_user.uid:
            if not fcb.check_permission(self.current_user.uid, self.current_user.gid, 0x02):
                return False
        if append:
            fcb.content += content
        else:
        ...
        def walk(fcb, base):
            ...
            for e in entries:
                ...
                if e['type'] == FCB.TYPE_DIRECTORY:
                    sub = FCB.from_dict(e)
                    walk(sub, base + '/' + e['name'] if base != '/' else '/' + e['name'])
        walk(self.root, "")
    ...
    def get_file_info(self, path):
        ...
        if not target:
            return "文件或目录不存在"
        lines = []
        lines.append(f"名称: {target.name}")
        lines.append(f"类型: {'目录' if target.type == FCB.TYPE_DIRECTORY else '文件'}")
        lines.append(f"大小: {target.size} 字节")
        lines.append(f"创建: {datetime.fromtimestamp(target.create_time).strftime('%Y-%m-%d %H:%M:%S')}")
        lines.append(f"修改: {datetime.fromtimestamp(target.modify_time).strftime('%Y-%m-%d %H:%M:%S')}")
        lines.append(f"权限: {target.get_permission_string()}")
        lines.append(f"所有者: {target.owner_uid}")
        return '\n'.join(lines)
    ...
    def import_file(self, external_path, fs_path):
        if not self.is_user_logged_in():
            return False
        if not os.path.exists(external_path) or not os.access(external_path, os.R_OK):
            return False
        try:
            ...
            return False
        if self._find_fcb(fs_path):
            old = self._find_fcb(fs_path)
            if old.type == FCB.TYPE_FILE:
                self.delete_file(fs_path)
            else:
                return False
        parent_path, name = self._split_path(fs_path)
        parent = self._find_fcb(parent_path)
        if not parent or parent.type != FCB.TYPE_DIRECTORY:
            return False
        fcb = FCB(name, FCB.TYPE_FILE, self.current_user.uid, self.current_user.gid)
    ...
    def get_system_info(self):
        ...
        count(self.root)
        return f"""=== 系统信息 ===
总空间: {self.get_total_space()//1024} KB
已用: {self.get_used_space()//1024} KB
空闲: {self.get_free_space()//1024} KB
用户: {self.get_current_username() or '未登录'}
路径: {self.cwd_path}
文件: {total_files}
目录: {total_dirs}
缓存: {len(self.cache)} 个文件"""

    # ---------- 缓存 ----------
    def _cache_get(self, path):
        if path in self.cache:
            self.cache.move_to_end(path)
            self.cache[path]['access'] += 1
    ...
    def _cache_put(self, path, content):
        if path in self.cache:
            self._cache_remove(path)
        size = len(content)
        while len(self.cache) >= self.MAX_CACHE or self.cache_mem + size > self.MAX_CACHE_MEM:
            self._cache_evict()
        self.cache[path] = {'content': content, 'access': 1}
        self.cache_mem += size
    def _cache_remove(self, path):
        if path in self.cache:
            self.cache_mem -= len(self.cache[path]['content'])
            del self.cache[path]
    ...
    def get_cache_stats(self):
        lines = [f"缓存文件: {len(self.cache)}/{self.MAX_CACHE}",
                 f"缓存内存: {self.cache_mem//1024} KB"]
        for p, d in self.cache.items():
            lines.append(f"  {p} (访问{d['access']}次)")
        return '\n'.join(lines)
    ...
    def scan_fragmentation(self):
        ...
        def walk(fcb, base):
            if fcb.type == FCB.TYPE_FILE and len(fcb.blocks) > 1:
                gaps = sum(1 for i in range(len(fcb.blocks)-1) if fcb.blocks[i+1]-fcb.blocks[i] > 1)
                ratio = gaps / len(fcb.blocks)
                if ratio > 0.1:
                    result.append((base+'/'+fcb.name, len(fcb.blocks), ratio))
            elif fcb.type == FCB.TYPE_DIRECTORY:
                for e in self._get_dir_entries(fcb):
                    walk(FCB.from_dict(e), base+'/'+fcb.name if base != '/' else fcb.name)
        walk(self.root, "")
        return result

    def defragment(self):
        frag = self.scan_fragmentation()
        if not frag:
            return "没有碎片文件"
        frag.sort(key=lambda x: x[2], reverse=True)
        lines = [f"发现 {len(frag)} 个碎片文件"]
        for path, blocks, ratio in frag:
            lines.append(f"  {path}: {blocks}块, {ratio*100:.1f}%")
        lines.append("整理完成（模拟）")
        self._save()
        return '\n'.join(lines)
    ...
    def _save(self):
        try:
            data = {
                ...
                'users': {uid: {'uid': u.uid, 'gid': u.gid, 'username': u.username,
                                'password': u.password, 'home': u.home, 'last_login': u.last_login}
    ...
    def _load(self):
        try:
            ...
            with open(self.data_file, 'r') as f:
                data = json.load(f)
            if data.get('root'):
                self.root = FCB.from_dict(data['root'])
                self.cwd = self.root
