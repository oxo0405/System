...
class FileSystemGUI(QMainWindow):
    ...
    def create_file_tab(self):
        ...
        self.file_content.setPlaceholderText("选择文件查看/编辑内容...")
        self.file_content.setStyleSheet("QTextEdit { background-color: #0a0a14; border: 1px solid #2d2d44; border-radius: 4px; font-family: 'Consolas', monospace; font-size: 13px; }")
        ...
        content_btn_layout.addWidget(save_btn)
        
        clear_btn = QPushButton("🗑 清空")
        clear_btn.clicked.connect(self.file_content.clear)
        content_btn_layout.addWidget(clear_btn)
        content_btn_layout.addStretch()
        content_layout.addLayout(content_btn_layout)
    ...
    def save_content(self):
        ...
        row = self.file_table.currentRow()
        if row < 0:
            self.show_error("错误", "请选择要保存的文件！")
            return
        
        name_item = self.file_table.item(row, 4)
        type_item = self.file_table.item(row, 5)
        if not name_item or "目录" in type_item.text():
            self.show_error("错误", "请选择文件进行保存！")
            return
    ...
    def search_file(self):
        ...
        if result != "文件未找到":
            ...
            count = len(lines)
            self.show_message(f"🔍 找到 {count} 个结果")
        else:
