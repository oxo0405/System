...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("文件系统 - 碎片整理+缓存版 (22)")
    ...
    def _create_opt_tab(self):
        ...
        g2l = QVBoxLayout(g2)
        desc2 = QLabel("基于LRU的缓存策略，自动缓存文件内容。")
        desc2.setStyleSheet("color:#a6adc8;")
        g2l.addWidget(desc2)
        h2 = QHBoxLayout()
        cache_refresh_btn = QPushButton("刷新缓存统计"); cache_refresh_btn.setObjectName("success"); cache_refresh_btn.clicked.connect(self._refresh_cache)
        h2.addWidget(cache_refresh_btn)
        clear_cache_btn = QPushButton("清空缓存"); clear_cache_btn.clicked.connect(self._clear_cache)
        h2.addWidget(clear_cache_btn)
        h2.addStretch()
        g2l.addLayout(h2)
        self.cache_stats = QTextEdit(); self.cache_stats.setMaximumHeight(100); self.cache_stats.setStyleSheet("background:#0f0f1a;")
        g2l.addWidget(self.cache_stats)
        layout.addWidget(g2)

        # 系统信息
        g3 = QGroupBox("系统信息")
        g3l = QVBoxLayout(g3)
        self.sys_info = QTextEdit(); self.sys_info.setReadOnly(True); self.sys_info.setMaximumHeight(80); self.sys_info.setStyleSheet("background:#0f0f1a;")
        g3l.addWidget(self.sys_info)
        refresh_sys_btn = QPushButton("刷新系统信息"); refresh_sys_btn.clicked.connect(self._refresh_sys)
        g3l.addWidget(refresh_sys_btn)
        layout.addWidget(g3)
    ...
    def refresh_all(self):
        self.refresh_files(); self.refresh_users(); self._update_user_info(); self._refresh_sys(); self._refresh_cache()
    ...
    # ---------- 碎片整理槽 ----------
    def _defrag(self):
        self.defrag_status.setText("正在扫描..."); self.defrag_status.setStyleSheet("color:#f9e2af;")
    ...
    def _do_defrag(self):
        try:
            ...
            self.defrag_status.setText("✅ 完成"); self.defrag_status.setStyleSheet("color:#a6e3a1;")
            self.status_bar.showMessage("碎片整理完成")
        except Exception as e:
            self.defrag_status.setText("❌ 错误"); self.defrag_status.setStyleSheet("color:#f38ba8;")
