...
class FileSystem:
    ...
    def get_system_info(self):
        ...
        count(self.root)
        return f"总文件: {total_files}\n总目录: {total_dirs}\n用户: {self.get_current_username() or '未登录'}\n路径: {self.cwd_path}\n缓存文件数: {len(self.cache)}"

    # ---------- 导入导出 ----------
    def import_file(self, external_path, fs_path):
        if not self.is_user_logged_in(): return False
        if not os.path.exists(external_path) or not os.access(external_path, os.R_OK): return False
        try:
            with open(external_path, 'rb') as f:
                raw = f.read()
                try: content = raw.decode('utf-8')
                except: content = raw.hex()
        except: return False
        if self._find_fcb(fs_path):
            old = self._find_fcb(fs_path)
            if old.type == FCB.TYPE_FILE: self.delete_file(fs_path)
            else: return False
        parent_path, name = self._split_path(fs_path)
        parent = self._find_fcb(parent_path)
        if not parent or parent.type != FCB.TYPE_DIRECTORY: return False
        fcb = FCB(name, FCB.TYPE_FILE, self.current_user.uid, self.current_user.gid)
        fcb.permission = 0o644; fcb.content = content; fcb.size = len(content); fcb.modify_time = time.time()
        self._add_entry(parent, fcb)
        # 可选：缓存导入的内容
        self._cache_put(fs_path, content)
        self._save()
        return True
    ...
    def export_file(self, fs_path, external_path):
        ...
        try:
            ...
            if all(c in '0123456789abcdefABCDEF' for c in content[:100]):
                try: raw = bytes.fromhex(content)
                except: raw = content.encode('utf-8')
            else: raw = content.encode('utf-8')
            with open(external_path, 'wb') as f: f.write(raw)
            return True
        except: return False
    ...
    def get_cache_stats(self):
        lines = [f"缓存文件数: {len(self.cache)}/{self.MAX_CACHE}",
                 f"缓存内存: {self.cache_mem//1024} KB/{self.MAX_CACHE_MEM//1024} KB"]
        if self.cache:
            lines.append("缓存列表:")
