...
class MainWindow(QMainWindow):
    ...
    def _login(self):
        ...
        if self.fs.login_user(u, p):
            self.user_status.setText("已登录: " + u)
            self.user_status.setStyleSheet("color: #a6e3a1;")
            self.refresh_all()
            self.status_bar.showMessage("欢迎 " + u)
        else:
    ...
    def _register(self):
        ...
        if self.fs.register_user(u, p):
            QMessageBox.information(self, "成功", "用户 " + u + " 注册成功！请登录。")
            self.reg_user.clear()
            self.reg_pass.clear()
    ...
    def _create_file_tab(self):
        ...
        ai_btn.setStyleSheet("background: #cba6f7; color: #1e1e2e;")
        ...
        self.search_results.itemDoubleClicked.connect(self._on_search_click)
    ...
    def _go_home(self):
        if self.fs.is_user_logged_in():
            self.fs.cd("/home/" + self.fs.get_current_username())
            self.refresh_files()
    ...
    def _open_file(self, name):
        ...
        self.file_name_label.setText("文件: " + name)
        ...
        if fd >= 0:
            ...
            lines = content.count('\n') + 1 if content else 0
            self.file_info_label.setText("行数: " + str(lines) + " | 大小: " + str(len(content)) + " 字节")
            self.status_bar.showMessage("已打开: " + name)
        else:
            QMessageBox.warning(self, "错误", "无法打开文件: " + name)
    ...
    def _mkdir(self):
        ...
        if ok and name.strip():
            ...
            if self.fs.mkdir(path):
                self.refresh_files()
                self.status_bar.showMessage("目录 " + name + " 创建成功")
            else:
    ...
    def _create_file(self):
        ...
        if ok and name.strip():
            ...
            if self.fs.create_file(path):
                ...
                self.status_bar.showMessage("文件 " + name + " 创建成功")
            else:
                QMessageBox.warning(self, "错误", "创建失败")
    ...
    def _delete(self):
        ...
        name = name_item.text()
        path = self.fs.cwd_path + "/" + name if self.fs.cwd_path != "/" else "/" + name
        reply = QMessageBox.question(self, "确认删除", "删除 '" + name + "' ?", QMessageBox.Yes|QMessageBox.No)
        if reply == QMessageBox.Yes:
            if not self.fs.delete_file(path):
                if not self.fs.rmdir(path):
                    ...
                    return
            self.refresh_files()
            self.status_bar.showMessage("已删除 " + name)
    ...
    def _import(self):
        ...
        if self.fs.import_file(fpath, fs_path):
            ...
            self.status_bar.showMessage("导入成功: " + fname)
        else:
    ...
    def _export(self):
        ...
        if self.fs.export_file(fs_path, save_path):
            self.status_bar.showMessage("导出成功: " + name)
        else:
    ...
    def _search(self):
        ...
        if result == "文件未找到":
            ...
            self.status_bar.showMessage("找到 " + str(len(lines)) + " 个结果")
    ...
    def _create_user_tab(self):
        ...
        self.gid_label = QLabel("-")
        info_layout.addRow("GID:", self.gid_label)
        self.home_label = QLabel("-")
        info_layout.addRow("主目录:", self.home_label)
    ...
    def _chmod(self):
        ...
        if self.fs.chmod(path, int(mode, 8)):
            self.status_bar.showMessage("权限已修改: " + path + " -> " + mode)
        else:
    ...
    def _create_opt_tab(self):
        ...
        self.defrag_btn.clicked.connect(self._defrag)
        h1.addWidget(self.defrag_btn)
        self.defrag_status = QLabel("就绪")
        self.defrag_status.setStyleSheet("color: #6c7086;")
        h1.addWidget(self.defrag_status)
        h1.addStretch()
        g1_layout.addLayout(h1)
        self.defrag_report = QTextEdit()
        self.defrag_report.setMaximumHeight(100)
        self.defrag_report.setStyleSheet("background: #0f0f1a;")
        ...
        self.cache_stats.setMaximumHeight(100)
        self.cache_stats.setStyleSheet("background: #0f0f1a;")
        ...
        self.sys_info.setStyleSheet("background: #0f0f1a;")
        g3_layout.addWidget(self.sys_info)
        refresh_sys = QPushButton("刷新系统信息")
        refresh_sys.clicked.connect(self._refresh_sys)
    ...
    def refresh_users(self):
        ...
        for u in self.fs.get_all_users():
            self.user_list.addItem(u.username + " (UID: " + str(u.uid) + ")")

    def _update_user_info(self):
        if self.fs.is_user_logged_in():
            user = self.fs.current_user
            self.user_info.setText("已登录: " + user.username)
